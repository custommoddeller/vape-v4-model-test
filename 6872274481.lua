...
runFunction(function()
	...
	local AlternateFlyMode = {Value = "CFrame"}
	local AlternateFlyVerticalSpeed = {Value = 40}
	...
	AlternateFly = GuiLibrary.ObjectsThatCanBeSaved.BlatantWindow.Api.CreateOptionsButton({
		...
		Function = function(callback)
			if callback then
				...
				table.insert(AlternateFly.Connections, inputService.InputBegan:Connect(function(input1)
					if AlternateFlyVertical.Enabled and inputService:GetFocusedTextBox() == nil then
						if input1.KeyCode == Enum.KeyCode.Space or input1.KeyCode == Enum.KeyCode.ButtonA then
							AlternateFlyUp = true
						end
						if input1.KeyCode == Enum.KeyCode.LeftShift or input1.KeyCode == Enum.KeyCode.ButtonL2 then
							AlternateFlyDown = true
						end
					end
				end))
				table.insert(AlternateFly.Connections, inputService.InputEnded:Connect(function(input1)
					if input1.KeyCode == Enum.KeyCode.Space or input1.KeyCode == Enum.KeyCode.ButtonA then
						AlternateFlyUp = false
					end
					if input1.KeyCode == Enum.KeyCode.LeftShift or input1.KeyCode == Enum.KeyCode.ButtonL2 then
						AlternateFlyDown = false
					end
				end))
				if inputService.TouchEnabled then
					pcall(function()
						local jumpButton = lplr.PlayerGui.TouchGui.TouchControlFrame.JumpButton
						table.insert(AlternateFly.Connections, jumpButton:GetPropertyChangedSignal("ImageRectOffset"):Connect(function()
							AlternateFlyUp = jumpButton.ImageRectOffset.X == 146
						end))
						AlternateFlyUp = jumpButton.ImageRectOffset.X == 146
					end)
				end
				...
				table.insert(AlternateFly.Connections, vapeEvents.AutoBankBalloon.Event:Connect(function()
					repeat task.wait() until getItem("balloon")
					inflateBalloon()
				end))
				...
				groundtime = tick() + (2.6 + (entityLibrary.groundTick - tick()))
				AlternateFlyCoroutine = coroutine.create(function()
					repeat
						repeat task.wait() until (groundtime - tick()) < 0.6 and not onground
						flyAllowed = ((lplr.Character and lplr.Character:GetAttribute("InflatedBalloons") and lplr.Character:GetAttribute("InflatedBalloons") > 0) or bedwarsStore.matchState == 2 or megacheck) and 1 or 0
						if (not AlternateFly.Enabled) then break end
						local Flytppos = -99999
						if flyAllowed <= 0 and AlternateFlyTP.Enabled and entityLibrary.isAlive then 
							local ray = workspace:Raycast(entityLibrary.character.HumanoidRootPart.Position, Vector3.new(0, -1000, 0), bedwarsStore.blockRaycast)
							if ray then 
								Flytppos = entityLibrary.character.HumanoidRootPart.Position.Y
								local args = {entityLibrary.character.HumanoidRootPart.CFrame:GetComponents()}
					...
					until (not AlternateFly.Enabled)
				end)
				coroutine.resume(AlternateFlyCoroutine)
				...
				RunLoops:BindToHeartbeat("AlternateFly", function(delta) 
					...
					if entityLibrary.isAlive then
						...
						local flyVelocity = entityLibrary.character.Humanoid.MoveDirection * (FlyMode.Value == "Normal" and AlternateFlySpeed.Value or 20)
						entityLibrary.character.HumanoidRootPart.Velocity = flyVelocity + (Vector3.new(0, playerMass + (AlternateFlyUp and AlternateFlyVerticalSpeed.Value or 0) + (AlternateFlyDown and -AlternateFlyVerticalSpeed.Value or 0), 0))
						if FlyMode.Value ~= "Normal" then
							entityLibrary.character.HumanoidRootPart.CFrame = entityLibrary.character.HumanoidRootPart.CFrame + (entityLibrary.character.Humanoid.MoveDirection * ((AlternateFlySpeed.Value + getSpeed()) - 20)) * delta
						end
					end
				end)
			else
				pcall(function() coroutine.close(AlternateFlyCoroutine) end)
				autobankballoon = false
				waitingforballoon = false
				lastonground = nil
				AlternateFlyUp = false
				AlternateFlyDown = false
				RunLoops:UnbindFromHeartbeat("AlternateFly")
				if FlyAnywayProgressBarFrame then 
					FlyAnywayProgressBarFrame.Visible = false
				end
				if AlternateFlyAutoPop.Enabled then
					if entityLibrary.isAlive and lplr.Character:GetAttribute("InflatedBalloons") then
						for i = 1, lplr.Character:GetAttribute("InflatedBalloons") do
							olddeflate()
						end
		...
		HoverText = "Boosts your fly speed at the start for 0.2s (spamming or frequent use can lead to lagbacks) Do not use on ping above 250",
		ExtraText = function() 
			return "Heatseeker"
		end
	})
	AlternateFlySpeed = AlternateFly.CreateSlider({
		Name = "Speed",
		Min = 1,
		Max = 23,
		Function = function(val) end, 
	...
	AlternateFlyVerticalSpeed = AlternateFly.CreateSlider({
		Name = "Vertical Speed",
		Min = 1,
		Max = 100,
		Function = function(val) end, 
	...
	AlternateFlyVertical = AlternateFly.CreateToggle({
		Name = "Y Level",
		Function = function() end, 
		Default = true
	})
	AlternateFlyAutoPop = AlternateFly.CreateToggle({
		Name = "Pop Balloon",
		Function = function() end, 
		HoverText = "Pops balloons when AlternateFly is disabled."
	})
	local oldcamupdate
	local camcontrol
	local Flydamagecamera = {Enabled = false}
	AlternateFlyDamageAnimation = AlternateFly.CreateToggle({
		Name = "Damage Animation",
		Function = function(callback) 
			if Flydamagecamera.Object then 
				Flydamagecamera.Object.Visible = callback
	...
	AlternateFlyTP = AlternateFly.CreateToggle({
		Name = "TP Down",
		Function = function() end,
		Default = true
	})
